...
class GuideMessageView extends LinearLayout {
    ...
    GuideMessageView(Context context) {
        ...
        final int padding = (int) (10 * density)*4;
        final int paddingBetween = (int) (3 * density);

        Typeface tf = Typeface.createFromAsset(getResources().getAssets(), "fonts/font_fs_blondescript_regular.otf");
        mTitleTextView = new TextView(context);
        mTitleTextView.setPadding(padding, padding, padding, padding);//(int)(padding*1.5)
        mTitleTextView.setGravity(Gravity.CENTER);
        mTitleTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 28);
