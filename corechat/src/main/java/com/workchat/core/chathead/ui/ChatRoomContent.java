...
import com.workchat.core.chat.ContentTypeInChat;
...
public class ChatRoomContent extends FrameLayout implements Content {
    ...
    public ChatRoomContent(@NonNull Context context, AddRoomEvent event) {
        super(context);
        this.roomId = event.getRoomId();
        this.roomAvatar = event.getRoomAvatar();
        init();
    ...
    private void init() {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.activity_chat_bubble, this, true);
...
//        txt.requestFocus();
    ...
    protected void onAttachedToWindow() {
        ...
        //neu adapter xay ra loi thi ko the load hinh, neu phat hien thi load lai room
        /*if (adapter != null && adapter.isHaveError()) {
            //load lai room
            adapter.setHaveError(false);
            ...
            //khi vào lại nếu room có tin chưa đọc thì gọi đánh dấu đã đọc
            updateLogViewed();
        }*/
...
//        EventBus.getDefault().unregister(this);
    ...
    private void setNumberUnread(int unread) {
        numberUnread = unread;
        EventBus.getDefault().post(new UpdateNumberEvent(roomId, numberUnread));
    }
...
//        MyUtils.showToastDebugInThread(context, "onShow()");
        //forcus lai socket, neu ko co ket noi thi goi ket noi lai
        initSocket();
...
//                isKeyboarShow = false;
    ...
    @BindView(R2.id.imgCloseRight)
    ImageView imgCloseRight;
    @BindView(R2.id.imgZoom)
    ImageView imgZoom;
    ...
    private void initUI() {
        ...
        if (!MyUtils.checkInternetConnection(context)) {
            if (emptyView != null) {
                ...
                emptyView.setTitleText(context.getString(R.string.khongCoInternet));
            }
        } else {
            if (emptyView != null) {
                emptyView.setImageState(R.drawable.ic_chat_square);
                emptyView.setTitleText(context.getString(R.string.empty_list_chat_notify));
            }
        }
...
//                            .setCameraPlaceholder(R.drawable.custom_camera)
                            ...
                            .pickPhoto(context, REQUEST_CODE_PICK_PICTURE);*/
                    /*Intent intent = new Intent(context, ChatHeadActivity.class);
                    intent.putExtra(ChatHeadActivity.IS_TAKE_PICTURE, true);
        ...
        //imgPin thay bang nut Close
        imgCloseRight.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                EventBus.getDefault().post(new CloseRoomEvent(roomId));
            }
        });
    ...
    private void requestStorage() {
        ...
        if (!isOK) {
            ActivityCompat.requestPermissions(context, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_REQUEST_STORAGE);
        }*/
    }


    //widget ko hien pin message
        ...
        /*if (isSocketConnected() && room != null) {
            ...
            socket.emit("getPinLogs", obj, new Ack() {
                ...
                public void call(Object... args) {
                    ChatRoomContent.this.post(new Runnable() {
                        @Override
                        public void run() {
    ...
    private void setupRecyclerView() {
        if (owner != null) {
            adapter = new ChatAdapter(
                    ChatApplication.instance.getApplicationContext(),
                    owner.get_id(), false, realm);
            adapter.setFromWidget(true);
            if (rv != null) {
                linearLayout = new LinearLayoutManager(context, RecyclerView.VERTICAL, true);
    ...
    private void whenHaveNewMessage(RoomLog log) {
        if ((room != null && room.get_id().equals(log.getRoomId())) ||
            ...
            if (adapter != null) {
                ...
                //neu dang mo thi moi la dang xem, nguoc lai thi xem nhu chua xem
                if (isShow) {
                        ...
                        if (isSuccess) {
                            ChatRoomContent.this.post(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    MyUtils.showToastDebug(context, "Application exists is " + ChatApplication.Companion.isExists());
                                }
                            });
    ...
    private void initRoomChatWithUser() {
        if (room == null) {
            if (isSocketConnected()) {
                ...
                if (obj.length() > 0) {
                    socket.emit("getPrivateRoom", obj, new Ack() {
                        @Override
                        public void call(Object... args) {
    ...
    private void initRoomChatProject(long projectId) {
        if (room == null) {
            if (isSocketConnected()) {
                ...
                if (obj.length() > 0) {
                    socket.emit("getProjectRoom", obj, new Ack() {
                        @Override
                        public void call(Object... args) {
                            room = Room.parseRoom(context, args);
                            whenHaveRoom();
                                ...
                                ChatRoomContent.this.post(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (code == Room.NOT_INT_ROOM_CODE) {
                                            String message = context.getString(R.string.not_in_room);
...
//                        MyUtils.log(obj.toString());
    ...
    private void setupMentionAdapter() {
        if (room != null) {
            ...
            mentionAdapter = new MentionAdapter(context, R.layout.mention_user_row_name_and_phone, list);
...
//                                            MyUtils.log("load more first" + list.size());
                                                    adapter.clearData();
                                                    adapter.append(list);
                                                    if (txtInternetState.getVisibility() == View.VISIBLE) {
                                                    ...
                                                    if (uri != null) {
                                                        ...
                                                        if (listImages != null && listImages.size() > 0) {
                                                            ...
                                                            for (int i = 0; i < listImages.size(); i++) {
                                                                ...
                                                                String mimeType = context.getContentResolver().getType(item);
...
//                                        adapter.notifyDataSetChanged();
        ...
        context.registerReceiver(networkChangeReceiver, new IntentFilter(NetworkChangeReceiver.ACTION_CONNECTIVITY_CHANGE));
    ...
    private void stateOnline() {
        ...
        txtInternetState.setText(context.getText(R.string.online_mode));
    ...
    private void stateOffline() {
        ...
        if (txtInternetState.getVisibility() == View.GONE) {
            if (animationOnline != null && animationOnline.isRunning()) {
                animationOnline.cancel();
            }
            txtInternetState.setText(context.getText(R.string.offline_mode));
    ...
    private boolean checkPermissionLocation() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            for (int i = 0; i < arrPermissionsLocation.length; i++) {
                int grant = ContextCompat.checkSelfPermission(context, arrPermissionsLocation[i]);
...
//                                MyUtils.showToast(context, R.string.upload_success);
    ...
    private void sendContact(long contactId,
                             String contactName,
...
//            builder.setTitle("Custom ChatView Alert Dialog");
            ...
            final View v = inflater.inflate(R.layout.activity_08_confirm_join_channel, null);
            ...
            txt2.setText(context.getString(R.string.number_members, room.getMembers().size()));
...
//                            videoPath = SiliCompressor.with(context).compressVideo(path, storageDir.getAbsolutePath());//,720,1280,1000
    ...
    private void intExpandItem() {
        ...
        linearWorkchat.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //mo room chat
...
//                        MyUtils.log("percent = " + percent);
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    private void showThongBaoNoInternet() {
                        ...
                        ChatRoomContent.this.post(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    int code = obj.getInt("errorCode");
                                    if (code == 0) {
                                        //1 item hoac ko co thi deu dc tao moi
                                        JSONArray data = obj.getJSONArray("data");
                                        if (data == null || data.length() == 0) {
                                            //tao
                                            inputDialogZoomMeeting();
                                        } else {
                                            JSONObject item = data.getJSONObject(0);
                                            LastLog log = new Gson().fromJson(item.toString(), LastLog.class);
                                            if (log != null) {
                                                JsonObject content = log.getContent();
                                                if (content != null) {
                                                    Meeting meeting = new Gson().fromJson(content.toString(), Meeting.class);
                                                    if (meeting != null) {
                                                        if (meeting.getStatus().equalsIgnoreCase("ended")) {
                                                            //tao
                                                            inputDialogZoomMeeting();
                                                        } else {
                                                            MyUtils.showAlertDialog(context, R.string.meeting_is_not_ended);
                                                        }
                                                    }
                                                }
                                            }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    private void inputDialogZoomMeeting() {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setView(R.layout.dialog_create_zoom_meeting);
        ...
        //phai goi sau khi alert show()
        ImageView meetingImgClose = alert.findViewById(R.id.meetingImgClose);
        AppCompatButton meetingBtnClose = alert.findViewById(R.id.meetingBtnClose);
        AppCompatButton meetingBtnCreate = alert.findViewById(R.id.meetingBtnCreate);
        AppCompatEditText meetingTxtTitle = alert.findViewById(R.id.meetingTxtTitle);
        AppCompatEditText meetingTxtPassword = alert.findViewById(R.id.meetingTxtPassword);

        //close
        meetingImgClose.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (alert != null)
                    alert.dismiss();
        ...
        meetingBtnClose.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                meetingImgClose.performClick();
            }
        });

        //create
        meetingBtnCreate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        if (MyUtils.checkInternetConnection(context)) {
            if (isSocketConnected() && !TextUtils.isEmpty(roomId) && room != null) {
                ...
                try {
                    ...
                    obj.put("userName", owner.getName());
