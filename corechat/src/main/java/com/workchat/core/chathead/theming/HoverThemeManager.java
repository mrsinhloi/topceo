...
public class HoverThemeManager implements HoverThemer {
    ...
    public static synchronized void init(@NonNull HoverTheme theme) {
        if (null == sInstance) {
            sInstance = new HoverThemeManager(theme);
        }
    }
    ...
    public HoverThemeManager( @NonNull HoverTheme theme) {
        setTheme(theme);
    }
    ...
    public void setTheme(@NonNull HoverTheme theme) {
        mTheme = theme;
        EventBus.getDefault().postSticky(theme);
    }
