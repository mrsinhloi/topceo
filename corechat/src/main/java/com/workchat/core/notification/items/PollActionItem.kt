...
class PollActionItem (context: Context, private val ntf: PollActionNotification) : BaseNotificationItem(context, ntf) {
    ...
    override fun bindView(holder: ViewHolder, payloads: MutableList<Any>) {
        ...
        holder.itemView.apply {
            ...
            if (ntf.data.actionData.roomAvatar==null || ntf.data.actionData.roomAvatar.contains("no-avatar.png")) {
                Glide.with(this).asBitmap().circleCrop().load(textBitmap(ntf.data.actionData.roomName.first().toString().capitalize())).into(ntfBase_avatar)
            } else {
                Glide.with(this).asBitmap().circleCrop().load(ntf.data.actionData.roomAvatar).into(ntfBase_avatar)
            }
