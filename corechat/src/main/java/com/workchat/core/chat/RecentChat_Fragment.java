 ...
 * Use the {@link RecentChat_Fragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class RecentChat_Fragment extends Fragment {
     ...
     */
    public static RecentChat_Fragment newInstance(int chatType, boolean isForward, boolean isSharing) {
        RecentChat_Fragment fragment = new RecentChat_Fragment();
...
//                        Log.i(TAG, "BOTTOM SCROLL");
        ...
        //init adapter
        //load chat gan day ALL truoc, cach vai giay moi load tiep cho 2 tab GROUP VA CHANNEL
        switch (chatType) {
            case RecentChatType.ALL:
    ...
    private class LoadDataTask extends AsyncTask<Void, Void, ArrayList<Room>> {
        ...
        protected void onPostExecute(ArrayList<Room> rooms) {
            ...
            if(getActivity()!=null && !getActivity().isFinishing()) {
                adapter = new RecentChat_Adapter(getActivity(), rooms, chatType);
                adapter.setCanLongClick(isCanLongClick);
    ...
    /*private void getContactList() {
        ...
        //chi lay o 1 man hinh
        if (chatType == RecentChatType.CHANNEL) {
            if (isSocketConnected()) {
                ...
                socket.emit("getContactList", obj, new Ack() {
                    @Override
                                    ...
                                    public void run() {
                                        ...
                                        if (list.size() > 0) {
                                            for (int i = 0; i < list.size(); i++) {
                                                list.get(i).setInChatContact(true);
                                            }
                                            listUsers.addAll(list);
                                        }

                                        //luu xuong bo nho, de khi vao man hinh Contacts se dong bo 2 danh ba
                                        db.putListUserInfo(UserInfo.LIST_USER_INFO_TEMP_CONTACT_LIST, listUsers);
                                    }
                                }).start();
...
//                            progressBar.setVisibility(ChatView.GONE);
                            }
    ...
    private void getData(final long lastLogDate) {
        ...
        boolean isLoading = false;
        if (isSocketConnected() && isNotFinishing()) {
            //ko load more
            switch (chatType) {
                case RecentChatType.ALL:
                    isLoading = isLoading1;
                                    ...
                                    if (list != null && list.size() > 0) {
                                        ...
                                        switch (chatType) {
                                            ...
                                            case RecentChatType.CHANNEL://type channel
                                                ...
                                                for (int i = 0; i < list.size(); i++) {
                                                    Room r = list.get(i);
                                                    if (r.getType()!=null && r.getType().equalsIgnoreCase(Room.ROOM_TYPE_CHANNEL)) {
                                                        listOfType.add(r);
                                                    }
                                                }
                                                //ADD VAO ADAPTER
...
//        if (chatType == RecentChatType.ALL) {
        //luu danh sach vao db 20 ptu
//        saveCache();

//        }
//        MyUtils.showToast(getContext(), "destroy");
...
//            MyUtils.howLong(start, "get list room from DB");
        } catch (Exception e) {
            e.printStackTrace();
        }
//        }
    ...
    public void onEventRoomHaveMessage(RoomLogEvent_Have_Message event) {
        ...
        if (log != null) {
            ...
            if (room != null) {
                ...
                if (log.getAuthorInfo()!=null && log.getAuthorInfo().getUserId().equals(user.get_id())) {
...
//            getContext().registerReceiver(receiver, new IntentFilter(Intent.ACTION_SCREEN_ON));
...
//        alertDialogBuilder.setIcon(R.drawable.fail);
    ...
    public void onResume() {
        ...
        //turn off screen and turn on screen again
        if(isInBackground){
            //lay lai tu dau
            getListRoomPin("", 4);
        }
