...
package com.topceo.mediaplayer.pip.utils;
...
import com.topceo.mediaplayer.pip.Consts;
...
//        final float aspectRatio = (float) video.getWidth() / (float) video.getHeight();
        final int thumbWidth = MyApplication.getInstance(context).getVideoThumbWidth();
...
//        final int thumbHeight = height > maxHeight ? maxHeight : height;
    ...
    public static void adjustVideoThumbView(@NonNull ImageView view) {
        final int thumbWidth = MyApplication.getInstance().getVideoThumbWidth();
    ...
    public static String concatVideoProgressAndDuration(int progress, int duration) {
        ...
        final boolean chinese = false;
                //"zh".equals(MyApplication.getInstance().getResources().getConfiguration().locale.getLanguage());
