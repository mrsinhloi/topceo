...
public class CommentAdapterSectionParent_ImageComment extends SectionedRecyclerViewAdapter<RecyclerView.ViewHolder> {
    private ArrayList<ImageComment> listItems = new ArrayList<>();

    public void setListItems(ArrayList<ImageComment> list) {
        this.listItems = list;
    }

    public void addListItems(ArrayList<ImageComment> list) {
        int sum = listItems.size();
        this.listItems.addAll(list);
        notifyItemRangeChanged(sum, list.size());
    }

    public void addItem(ImageComment item, int position) {
     ...
     * @param obj
     */
    public void updateItem(ImageComment obj) {
    ...
    public int getParentAndChildCount() {
        int count = 0;
        if (listItems != null && listItems.size() > 0) {
            for (int i = 0; i < listItems.size(); i++) {
                ImageComment item = listItems.get(i);
                count += item.getReplyCount() + 1;//1:parent+reply:child
            }
        }
        return count;
    }
    ...
    public void add(int position, ImageComment comment) {
        if (listItems != null) {
    ...
    private int findPostionParent(long id) {
        ...
        if (listItems != null && listItems.size() > 0) {
            for (int i = 0; i < listItems.size(); i++) {
                ImageComment c = listItems.get(i);
                if (c.getItemId() == id) {
                    position = i;
                    break;
                }
    ...
    public void add(ImageComment comment) {
        if (listItems != null) {
    ...
    public CommentAdapterSectionParent_ImageComment(ArrayList<ImageComment> horizontalList, Context context, boolean isMyPost) {
    ...
    public void onBindHeaderViewHolder(RecyclerView.ViewHolder holder1, int section) {
        ...
        //set image
        if (item.getUser() != null && !TextUtils.isEmpty(item.getUser().getAvatarSmall())) {

            Glide.with(context)
                    .load(item.getUser().getAvatarSmall())
...
//                Intent intent = new Intent(context, CommentFacebookActivityChild.class);
//                intent.putExtra(ImageComment.COMMENT_OBJECT, item);
//                context.startActivity(intent);
                EventBus.getDefault().post(new EventImageComment(item));
            }
        });
        ...
        if (item.getLikeCount() > 0) {
            ...
            holder.txt4.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra(ImageComment.COMMENT_ID, item.getItemId());
                    context.startActivity(intent);
                }
            });
        } else {
...
//                    listItems.get(section).setShowCommentChild(!item.isShowCommentChild());
                    ...
                    try {
                        if (item.isShowCommentChild()) {//xoa list dang co, load lai tu dau, set text hide
                            ...
                            //load childs
                            getComments(item.getImageItemId(), item.getItemId(), 0, 0);
                            //set text la an
                            holder.txtReplyMoreCount.setText(R.string.hide_reply_number);
                        } else {
...
//                            notifyItemChanged(section);
        ...
        if (isMyPost || (item.getUser() != null && user.getUserId() == item.getUser().getUserId())) {
...
//                    dialog.setTitle(title);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder1, int section, int relativePosition, int absolutePosition) {
        if (listItems.get(section).getListChild().size() > 0) {
            ...
            //set image
            if (!TextUtils.isEmpty(item.getUser().getAvatarSmall())) {
                Glide.with(context)
                        .load(item.getUser().getAvatarSmall())
                        .override(width, width)
                        .diskCacheStrategy(DiskCacheStrategy.ALL)
...
//                    dialog.setTitle(title);
    ...
    private void getComments(final long imageItemId, final long replyToId, final long lastItemId, final long lastCreateDate) {

        if (MyUtils.checkInternetConnection(context)) {
            Webservices.getComments(imageItemId, replyToId, lastItemId, lastCreateDate).continueWith(new Continuation<Object, Void>() {
                @Override
...
//                                        notifyItemChanged(position);
                                    ...
                                    ImageComment last = comments.get(comments.size() - 1);
                                    if (last != null) {
                                        getComments(imageItemId, replyToId, last.getItemId(), last.getCreateDate());
                                    }
                                }
                            ...
                            MyApplication.initCookie(context).continueWith(new Continuation<Object, Void>() {
                                ...
                                public Void then(Task<Object> task) throws Exception {
                                    if (task.getResult() != null) {
                                        ...
                                        if (kq != null) {
                                            getComments(imageItemId, replyToId, lastItemId, lastCreateDate);
                                        }
                                    }
                                    return null;
                                }
...
//                                MyUtils.showToastDebug(context, "Liked...");
    ...
    /*public void setLikeComment(long commentId, boolean isLiked) {
        if(isLiked){
            MyApplication.apiManager.commentLike(commentId, new Callback<JsonObject>() {
                @Override
            ...
            MyApplication.apiManager.commentUnLike(commentId, new Callback<JsonObject>() {
                @Override
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    JsonObject obj = response.body();
                    parseJson(obj.toString(), commentId, isLiked);
    ...
    private void deleteComment(final long commentId, int section, int positionChild) {

        if (MyUtils.checkInternetConnection(context)) {
            Webservices.deleteComment(commentId).continueWith(new Continuation<Object, Void>() {
                @Override
                public Void then(Task<Object> task) throws Exception {
                    if (task.getError() == null) {
                        if (task.getResult() != null) {
                            ...
                            if (result) {
                                ...
                                context.sendBroadcast(new Intent(MH02_PhotoDetailActivity.ACTION_COMMENT_DELETED));
                            }
                        }
                    } else {
                        boolean isLostCookie = MyApplication.controlException((ANError) task.getError());
                            ...
                            MyApplication.initCookie(context).continueWith(new Continuation<Object, Void>() {
                                ...
                                public Void then(Task<Object> task) throws Exception {
                                    if (task.getResult() != null) {
                                        User kq = (User) task.getResult();
                                        if (kq != null) {
                                            deleteComment(commentId, section, positionChild);
                                        }
                                    }
                                    return null;
                                }
