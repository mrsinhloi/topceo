...
public class ColorFilterGenerator {
	...
	public static void adjustHue(ColorMatrix cm, float value) {
		...
		float lumG = 0.715f;
		float lumB = 0.072f;
		float[] mat = new float[] { lumR + cosVal * (1 - lumR) + sinVal * (-lumR), lumG + cosVal * (-lumG) + sinVal * (-lumG),
	...
	public static void adjustContrast(ColorMatrix cm, int value) {
		...
		float[] mat = new float[] { x / 127, 0, 0, 0, 0.5f * (127 - x), 0, x / 127, 0, 0, 0.5f * (127 - x), 0, 0, x / 127, 0, 0.5f * (127 - x), 0, 0, 0, 1, 0,
				0, 0, 0, 0, 1 };
		cm.postConcat(new ColorMatrix(mat));
	...
	public static void adjustSaturation(ColorMatrix cm, float value) {
		...
		float[] mat = new float[] { lumR * (1 - x) + x, lumG * (1 - x), lumB * (1 - x), 0, 0, lumR * (1 - x), lumG * (1 - x) + x, lumB * (1 - x), 0, 0,
