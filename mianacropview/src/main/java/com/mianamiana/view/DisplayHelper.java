...
//        mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();
    ...
    public Bitmap getCroppedBitmap() {
        ...
        //kich thuoc cua bitmap nguon
        //neu hinh lon hon ORIGINAL_WIDTH thi resize
        int width = mSrcBitmapWidth;//mCropArea.width()
        int height = mSrcBitmapHeight;//mCropArea.height()

        if (!mIsFillMode) {
            if (width > ImageSize.ORIGINAL_WIDTH) {
                float ratio = (float) height / width;

                //resize
                width = ImageSize.ORIGINAL_WIDTH;
                height = (int) (width * ratio);

                Bitmap output = Bitmap.createScaledBitmap(mSrcBitmap, width, height, true);
                return output;

            } else {
                //giu nguyen kich thuoc goc
                return mSrcBitmap;
            }
        } else {//square
            /*if (width > ImageSize.ORIGINAL_WIDTH) {
                width = ImageSize.ORIGINAL_WIDTH;
                height = ImageSize.ORIGINAL_WIDTH;
            } else {
                width = Math.round(mCropArea.width());
                height = Math.round(mCropArea.height());
            }*/
